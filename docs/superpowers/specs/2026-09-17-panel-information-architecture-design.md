...
- [ ] 面板有清晰的三个层级:工作空间列表(分页)→ 工作空间详情(采集信息 + 需求列表)→ 任务详情(任务状态 + 轨迹 + 会话跳转)。
- [ ] 主列表每页最多 5 个工作空间,超过分页。
- [ ] 每个工作空间行 / 详情页显示**它自己的** TAPD 采集状态(上次同步时间、成功/失败、新增几条),不再只有全局一个时间戳。
- [ ] 顶部不再有「已同步 · 刚刚」这条正常态废话;面板数据出错时才显示警示条。
- [ ] 任务详情页完整呈现:阶段 gauge、中断原因、facts、验收标准、产物时间线、**轨迹时间线**、**可点会话跳转**。
- [ ] `npm run test:ui` / `test:panel-route`(即 `test:route`)/ `test:client-half` / `test:watch` / `test:stage` / `test:recover` 通过;`npm run lint` 干净。
