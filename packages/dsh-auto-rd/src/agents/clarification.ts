 ...
 *
 * Note: the old B-4 HARD-GATE (park on any unresolved question) has been
 * replaced by the separate Resolution role — Clarification ASKS, Resolution
 * ANSWERS (see resolution.ts).
 */
...
export class ClarificationAgent implements AgentSpec {
  ...
  readonly toolFilter = {
    allow: ['fs_read', 'fs_search', 'fs_glob', 'web_fetch', 'fs_write'],
  }
}